...
class WC_BPost_Shipping_Api_Label {
	...
	public function get_label( $order_reference, $format, $with_return_labels ) {
		...
		if ( 1 !== count( $labels ) ) {
			...
			switch ( count( $labels ) ) {
				case 0:
					throw new WC_BPost_Shipping_Label_Exception_Not_Found( sprintf(
						bpost__( 'Label not found for reference=%s, format=%s, with_return_label=%d.' ),
						$order_reference, $format, $with_return_labels
					) );
				case 1:
					break;
				default:
					throw new WC_BPost_Shipping_Label_Exception_Too_Much_Found( sprintf(
						bpost__( '%d labels found for reference=%s, format=%s, with_return_label=%d.' ),
						count( $labels ), $order_reference, $format, $with_return_labels
					) );
			}
		}
