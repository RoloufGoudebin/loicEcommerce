	...
	public function environment_check() {
		try {
			...
			if ( ! $this->product_configuration->has_configured_products() ) {
				$this->add_error( sprintf( bpost__(
					'%s is enabled, but there is not configured product. Please verify the API URL, and your Shipping Manager (%s)'
				), 'bpost', 'https://www.bpost.be/ShmBackEnd/private' ) );
			}
	...
	private function handle_exception( BpostException $exception ) {
		...
		switch ( $exception->getCode() ) {
			case 401: // Invalid response
				$this->add_error( sprintf( bpost__(
					'The credentials seem incorrect. Please verify them on your Shipping Manager (%s)'
				), $shm_url ) );
				break;
			...
			case 6: // Could not resolve host
				$this->add_error( sprintf( bpost__(
					'The URL seems incorrect (error %s). Please verify it in your Shipping Manager (%s)'
				), $exception->getCode(), $shm_url ) );
				break;
